...
const ScrollExpandMedia = ({
  ...
  const initialSquarePx = Math.max(50, Math.min(containerHeightPx, containerWidthPx) * initialCircleScale);
  ...
  const cornerRadiusPx = startCornerPx + (endCornerPx - startCornerPx) * scrollProgress;
  ...
  const endPercent = 50;
  const topPercent = startPercent + (endPercent - startPercent) * scrollProgress;
  const leftPercent = startPercent + (endPercent - startPercent) * scrollProgress;
            ...
            <div className="flex flex-col items-center justify-center w-full h-[100dvh] relative">
              ...
              <div
                ...
                style={{
                  ...
                  filter: scrollProgress > 0.7 ? "drop-shadow(0 10px 30px rgba(0,0,0,0.35))" : "none",
                }}
                ref={wrapperRef}
              >
                {mediaType === "video" ? (
...
const MediaContent = ({ mediaType }: { mediaType: "video" | "image" }): React.ReactElement => {
  const currentMedia = sampleMediaContent[mediaType];

  return (
    <div className="max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold mb-6 text-white">Om komponenten</h2>
      <p className="text-lg mb-8 text-gray-200">{currentMedia.about.overview}</p>
      <p className="text-lg mb-8 text-gray-200">{currentMedia.about.conclusion}</p>
    </div>
  );
};
